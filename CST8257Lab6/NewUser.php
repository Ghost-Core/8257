<?php
    ...
    $hashedPassword = hash("sha256",$_POST["passwordTxt"]);
    ...
    if(isset($_POST['submit']))
    {
        ...
        {            
            ...
            if ($chkAccount['StudentId'] == "") //user does not exist
            {
                ...
                $pStmt->execute(array(':idTxt' => $studentIdTxt, ':nameTxt' => $nameTxt, ':phoneNumberTxt' => $phoneNumberTxt, ':passwordTxt' => $hashedPassword));
                $pStmt->commit;      
...
<div class='form-group row'>
    ...
    <input type='password' class='form-control' id='passwordTxt'  value='<?php print $_SESSION['passwordTxt'];?>' name='passwordTxt' ></div>
                <div>
                <label class='col-form-label'>Show Password &nbsp<input type="checkbox" class="checkbox-inline" onclick="myFunction()"></label>
            </div>
